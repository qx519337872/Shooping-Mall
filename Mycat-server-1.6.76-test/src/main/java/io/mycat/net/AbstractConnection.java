    ...
    public void onReadData(int got) throws IOException {
        ...
        for (; ; ) {
            ...
            if (position >= offset + length && readBuffer != null) {
                ...
                //判断是是否开启流式查询
                if (MycatServer.getInstance().getConfig().getSystem().getuseStreaming()) {
                    //如果开启了流式查询，则进入for循环，循环判断当前使用内存是否超过最大内存的0.75
                    //如果超过，则程序阻塞，如果没有，则继续执行，并且跳出循环
                    for (; ; ) {
                        memoryMXBean = ManagementFactory.getMemoryMXBean();
                        if (memoryMXBean.getHeapMemoryUsage().getUsed() < (memoryMXBean.getHeapMemoryUsage().getMax() * 0.7)) {
                            handle(data);
                            break;
                        }
                    }
